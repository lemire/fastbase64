...
void testencode(const char *data, size_t datalength, bool verbose) {
  ...
  BEST_TIME_NOCHECK(memcpy(buffer, data, datalength), , repeat, datalength,
                    verbose);
  BEST_TIME_NOCHECK(linux_base64_encode(buffer, data, data + datalength), ,
                    repeat, datalength, verbose);
  BEST_TIME_NOCHECK(quicktime_base64_encode(buffer, data, datalength), , repeat,
                    datalength, verbose);
  BEST_TIME(chromium_base64_encode(buffer, data, datalength), (int)expected, ,
            repeat, datalength, verbose);
  BEST_TIME_CHECK(scalar_base64_encode(data, datalength, buffer, &outputlength),
                  (outputlength == expected), , repeat, datalength, verbose);

#ifdef HAVE_AVX2
  BEST_TIME_CHECK(
      klomp_avx2_base64_encode(data, datalength, buffer, &outputlength),
      (outputlength == expected), , repeat, datalength, verbose);
  BEST_TIME_CHECK(fast_avx2_base64_encode(buffer, data, datalength),
                  (int)expected, , repeat, datalength, verbose);
#endif // HAVE_AVX2
#ifdef HAVE_AVX512BW
  BEST_TIME_CHECK(fast_avx512bw_base64_encode(buffer, data, datalength),
                  (int)expected, , repeat, datalength, verbose);
#endif // HAVE_AVX512BW
#ifdef ALTIVEC
  BEST_TIME_CHECK(altivec_base64_encode(buffer, data, datalength),
                  (strlen(buffer) == expected), , repeat, datalength, verbose);
#endif // ALTIVEC
...
void testdecode(const char *data, size_t datalength, bool verbose) {
  ...
  BEST_TIME_NOCHECK(memcpy(buffer, data, datalength), , repeat, datalength,
                    verbose);
  BEST_TIME(linux_base64_decode(buffer, data, data + datalength), (int)expected,
            , repeat, datalength, verbose);
  BEST_TIME(quicktime_base64_decode(buffer, data), (int)expected, , repeat,
            datalength, verbose);
  BEST_TIME(chromium_base64_decode(buffer, data, datalength), (int)expected, ,
            repeat, datalength, verbose);
  // scalar_base64_decode and klomp_avx2_base64_decode return 0/1
  int scalarexpected = scalar_base64_decode(data, datalength, buffer, &outputlength);
  assert(outputlength == expected);
  BEST_TIME(scalar_base64_decode(data, datalength, buffer, &outputlength),
            scalarexpected, , repeat, datalength, verbose);

#ifdef HAVE_AVX2
  BEST_TIME(klomp_avx2_base64_decode(data, datalength, buffer, &outputlength),
            scalarexpected, , repeat, datalength, verbose);
  BEST_TIME(fast_avx2_base64_decode(buffer, data, datalength), (int)expected, ,
            repeat, datalength, verbose);
#endif // HAVE_AVX2
#ifdef HAVE_AVX512BW
  BEST_TIME(fast_avx512bw_base64_decode(buffer, data, datalength),
            (int)expected, , repeat, datalength, verbose);
#endif // HAVE_AVX512BW
#ifdef ALTIVEC
  BEST_TIME_CHECK(altivec_base64_decode(buffer, data, datalength),
                  (int)expected, , repeat, datalength, verbose);
#endif // ALTIVEC
  free(buffer);
  if (verbose)
...
#ifdef HAVE_AVX2
  printf("#displaying cycles per input bytes for linux, quicktime, chromium, "
         "scalar and avx2 decoders, first column is number of bytes\n");
#endif // HAVE_AVX2
#ifdef HAVE_AVX512BW
  printf("#displaying cycles per input bytes for linux, quicktime, chromium, "
         "scalar, avx2 and avx512 decoders, first column is number of bytes\n");
#endif // HAVE_AVX512BW
#ifdef ALTIVEC
  printf("#displaying nanoseconds per input bytes for linux, quicktime, chromium, "
         "scalar and altivec decoders, first column is number of bytes\n");
#endif // ALTIVEC
...
#ifdef HAVE_AVX2
  printf("#displaying cycles per input bytes for linux, quicktime, chromium, "
         "scalar and avx2 encoders, first column is number of bytes\n");
#endif // HAVE_AVX2
#ifdef HAVE_AVX512BW
  printf("#displaying cycles per input bytes for linux, quicktime, chromium, "
         "scalar, avx2 and avx512 encoders, first column is number of bytes\n");
#endif // HAVE_AVX512BW
#ifdef ALTIVEC
  printf("#displaying nanoseconds per input bytes for linux, quicktime, chromium, "
         "scalar and altivec encoders, first column is number of bytes\n");
#endif // ALTIVEC
  for (int l = 8; l <= N; l++) {
    printf("%d ", l);
