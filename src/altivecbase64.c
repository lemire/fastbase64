...
static inline __m128i enc_translate(const __m128i in) {
    const __m128i lut = {65, 71, -4, -4, -4, -4, -4, -4, -4, -4, -4, -4, -19, -16, 0, 0};
    __m128i indices = vec_subs(in, _mm_set1_epi8(51));
    const __m128i mask = vec_cmpgt((__vector char) in, (__vector char) _mm_set1_epi8(25));
...
static inline __m128i dec_reshuffle(__m128i in) {
    ...
    const __m128i only_c_and_a_shifted = vec_sl((__v8hu) only_c_and_a, (__v8hu) _mm_set1_epi16(6));
    ...
    return vec_perm(abcd, abcd, (__m128i) {2, 1, 0, 6, 5, 4, 10, 9, 8, 14, 13, 12, -1, -1, -1, -1});
}
...
size_t altivec_base64_decode(char *out, const char *src, size_t srclen) {
    ...
    // ensure that there are at least 6 valid bytes of input data remaining to close
    // the gap. 16 + 2 + 6 = 24 bytes:
    while (srclen >= 24) {
        ...
        const __m128i roll  = vec_perm(lut_roll, lut_roll, vec_add(eq_2F, hi_nibbles));
